 ...
 * Produces internally-consistent, clearly-fake demo data clustered
 * around "today". Fixed PRNG seed ensures resetDemo() restores identical state.
 *
 * RULES (plan §11): no real PII; @example.com emails only; cross-linked
 * entities; today-relative dates; no Math.random(); no live network calls.
 *
 * V1 TARGETS (approximate):
...
const INDUSTRIES = [
  ...
  'Beauty & Wellness', 'Automotive', 'Finance', 'Photography', 'Education', 'Veterinary',
];
...
export function generateDemoData(): DemoData {
  ...
  // ---- Conversations + Messages (35 threads, 8-15 msgs each) ----
  ...
  // Pick 35 unique contacts for conversations (stable selection via PRNG)
  ...
  for (let i = 0; i < contactPool.length && convContacts.length < 35; i++) {
    ...
    const tmp = contactPool[i];
    contactPool[i] = contactPool[idx];
    contactPool[idx] = tmp;
  ...
  // ---- Pipelines (4) + Opportunities (~96 total) ----
  const pipelines: Pipeline[] = [
    ...
    },
    {
      ...
      stages: ['Cold', 'Warming Up', 'Hot', 'Converted'].map((name, o) => ({ id: `st_n_${o}`, name, order: o })),
    },
  ];
  ...
  // ---- Calendars (3) + Appointments (55, clustered around today) ----
  ...
  const workflows: Workflow[] = [
    ...
    { id: 'wf_12', name: 'Referral Thank-you', status: 'published', enrolled: int(15, 60), trigger: 'Tag added: referred-by', explanation: 'Sends a thank-you note to the referrer whenever a new client is tagged as a referral.' },
  ];

  // ---- Campaigns (10 email + 8 SMS = 18 total) ----
  const campaigns: Campaign[] = [
    ...
    { id: 'cmp_s4', type: 'sms', name: 'Review Request Blast', status: 'sent', audienceSize: 450, sentAt: iso(now() - 12 * DAY), metrics: { delivered: 443, replyRate: 0.08, optOutRate: 0.005 }, content: { body: 'We loved working with you! Mind leaving us a quick review? {link}' } },
  ...
  const notifications: Notification[] = [
    ...
    { id: 'n_10', type: 'review', title: 'New 4-star review', body: 'A new Facebook review just came in from Noah Mercer.', createdAt: iso(now() - 1 * DAY - 2 * HOUR), read: true, link: '/reputation' },
