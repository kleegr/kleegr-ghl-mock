...
import { Mail, MessageSquare, Plus, Send, Copy, BarChart2, Layout } from 'lucide-react';
import { useStore } from '@/store/useStore';
import {
  PageHeader, Button, Badge, Card, Tabs, EmptyState,
...
function CampaignWizard({
  ...
  open: boolean;
  onClose: () => void;
  pushToast: (t: { title: string; description?: string; variant: 'success' | 'default' | 'info' }) => void;
}) {
  const [w, setW] = useState<WizardState>(WIZARD_DEFAULTS);
  const update = (patch: Partial<WizardState>) => setW((s) => ({ ...s, ...patch }));

  const handleSubmit = () => {
    pushToast({
      title: 'Demo: campaign queued',
      description: `"${w.name || 'Untitled'}" added to queue (demo only — nothing sent).`,
      variant: 'success',
    });
    onClose();
    setW(WIZARD_DEFAULTS);
  };
      ...
      footer={
        <div className="flex w-full items-center justify-between">
          ...
          <div className="flex gap-2">
            {w.step > 1 && (
              <Button variant="secondary" size="sm" onClick={() => update({ step: (w.step - 1) as 1 | 2 | 3 })}>
                Back
              </Button>
            )}
            {w.step < 3 ? (
              <Button size="sm" onClick={() => update({ step: (w.step + 1) as 1 | 2 | 3 })}>
                Next
              </Button>
            ) : (
              <Button size="sm" data-tour="marketing.campaignSubmit" onClick={handleSubmit}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['email', 'sms'] as const).map((t) => (
                <button
                  key={t}
                  onClick={() => update({ campaignType: t })}
                  className={`rounded-xl border-2 p-4 text-left transition-colors ${
                    w.campaignType === t ? 'border-brand bg-brand-soft' : 'border-line bg-surface hover:border-brand/40'
                  }`}
                >
                  <div className="mb-2 flex items-center gap-2">
                    {t === 'email' ? <Mail size={18} className="text-brand" /> : <MessageSquare size={18} className="text-brand" />}
                    <span className="text-sm font-bold text-ink capitalize">{t}</span>
                  </div>
                  <p className="text-xs text-ink-muted">
                    {t === 'email' ? 'Rich HTML email to a contact list' : 'Text message blast with reply tracking'}
                  </p>
                </button>
              ))}
            </div>
            <div className="space-y-1">
              ...
              <input
                className="w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-brand focus:ring-1 focus:ring-brand/30"
          ...
          <div className="space-y-4">
            ...
            {w.campaignType === 'email' && (
              <div className="space-y-1">
                ...
                <input
                  className="w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-brand focus:ring-1 focus:ring-brand/30"
                  placeholder="e.g. 🌟 Something special inside"
                  value={w.subject}
            ...
            <div className="space-y-1">
              <label className="block text-xs font-semibold text-ink-muted">
                {w.campaignType === 'email' ? 'Email body' : 'Message text'}
              </label>
              <textarea
                rows={5}
                className="w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink outline-none focus:border-brand focus:ring-1 focus:ring-brand/30"
          ...
          <div className="space-y-4">
                ...
                {[{ id: 'now', label: 'Send now', desc: 'Deliver immediately' }, { id: 'schedule', label: 'Schedule', desc: 'Pick a date & time' }].map((s) => (
                  <button
                    ...
                    className={`rounded-xl border-2 p-3 text-left transition-colors ${
                      w.schedule === s.id ? 'border-brand bg-brand-soft' : 'border-line bg-surface hover:border-brand/40'
                    }`}
                  >
            ...
            <div className="rounded-xl border border-line bg-surface-sunken p-4 text-sm">
              ...
              <div className="space-y-1 text-ink-muted">
                <p><span className="font-medium text-ink-muted">Type:</span> {w.campaignType}</p>
                <p><span className="font-medium text-ink-muted">Name:</span> {w.name || 'Untitled'}</p>
                <p><span className="font-medium text-ink-muted">Audience:</span> {w.audience}</p>
                {w.subject && <p><span className="font-medium text-ink-muted">Subject:</span> {w.subject}</p>}
                <p><span className="font-medium text-ink-muted">Schedule:</span> {w.schedule === 'now' ? 'Send immediately' : 'Scheduled'}</p>
              </div>
            </div>
            <p className="rounded-lg bg-warn/10 px-3 py-2 text-xs font-medium text-warn">
              Demo mode — no real emails or texts will be sent.
            </p>
          </div>
        )}
      </div>
  ...
  const barData =
    campaign.type === 'email'
      ? [
          ...
          { label: 'Opened', value: Math.round((campaign.metrics.openRate ?? 0) * delivered) },
          { label: 'Clicked', value: Math.round((campaign.metrics.clickRate ?? 0) * delivered) },
        ]
      : [
          { label: 'Delivered', value: delivered },
          { label: 'Replied', value: Math.round((campaign.metrics.replyRate ?? 0) * delivered) },
          { label: 'Opted out', value: Math.round((campaign.metrics.optOutRate ?? 0) * delivered) },
        ];
      ...
      footer={
        <>
          <Button
            variant="secondary"
            size="sm"
            onClick={() => pushToast({ title: 'Demo: duplicated', variant: 'info' })}
          >
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              pushToast({ title: 'Demo: test sent', description: 'Test sent to your address (demo only).', variant: 'info' })
            }
          >
    ...
    >
      <div data-tour="marketing.campaignDetail" className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          <Badge
            tone={campaign.status === 'sent' ? 'good' : campaign.status === 'scheduled' ? 'warn' : 'neutral'}
          >
            {campaign.status}
          </Badge>
          ...
          {campaign.sentAt && (
            <span className="text-xs text-ink-subtle">Sent {dateLabel(campaign.sentAt)}</span>
          )}
        </div>
        ...
        {campaign.content.subject && (
          <p className="text-sm">
            ...
            <span className="text-ink-muted">{campaign.content.subject}</span>
          </p>
        )}
...
const TEMPLATE_LIST = [
  { name: 'Welcome Email',        type: 'email', uses: 14 },
  { name: 'Follow-up SMS',        type: 'sms',   uses: 28 },
  { name: 'Promo Blast',          type: 'email', uses: 7  },
  { name: 'Appointment Reminder', type: 'sms',   uses: 42 },
  { name: 'Re-engagement',        type: 'email', uses: 5  },
  { name: 'Review Request',       type: 'sms',   uses: 33 },
];
        ...
        >
          <div className="flex items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-lg bg-brand-soft text-brand">
              {t.type === 'email' ? <Mail size={16} /> : <MessageSquare size={16} />}
            </span>
            <div>
              <p className="text-sm font-semibold text-ink">{t.name}</p>
              <p className="text-xs text-ink-subtle">{t.uses} uses</p>
            </div>
          </div>
        </Card>
      ))}
...
export function Marketing({ type }: { type: 'email' | 'sms' }) {
  ...
  const [localTabOverride, setLocalTabOverride] = useState<string | null>(null);
  const [statusFilter, setStatusFilter]         = useState<string>('all');
  const [preview, setPreview]                   = useState<Campaign | null>(null);
  const [wizardOpen, setWizardOpen]             = useState(false);

  // activeTab: explicit override (for Templates) OR the route-driven prop
  const activeTab = localTabOverride ?? type;
  ...
  const avgOpen   = sent.length
    ? sent.reduce((s, c) => s + (type === 'email' ? (c.metrics.openRate ?? 0) : (c.metrics.replyRate ?? 0)), 0) / sent.length
    : 0;
  const avgClick  = sent.length
    ? sent.reduce((s, c) => s + (c.metrics.clickRate ?? 0), 0) / sent.length
    : 0;

  const handleTabChange = (id: string) => {
    if (id === 'templates') {
      setLocalTabOverride('templates');
      return;
    }
    setLocalTabOverride(null);
  ...
  const columns: Column<Campaign>[] = [
      ...
      render: (c) => (
        <div className="flex items-center gap-2.5" data-tour="marketing.campaignRow">
          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-brand-soft text-brand">
            {c.type === 'email' ? <Mail size={14} /> : <MessageSquare size={14} />}
          </span>
    ...
    },
    {
      ...
      header: 'Status',
      render: (c) => (
        <Badge tone={c.status === 'sent' ? 'good' : c.status === 'scheduled' ? 'warn' : 'neutral'}>
          {c.status}
        </Badge>
      ),
    },
    {
      key: 'audience',
      header: 'Audience',
      render: (c) => <span className="text-ink-muted">{c.audienceSize.toLocaleString()}</span>,
    },
    {
      key: 'openRate',
      header: type === 'email' ? 'Open rate' : 'Reply rate',
      render: (c) => (
        <span className="text-ink-muted">
          {type === 'email'
            ? c.metrics.openRate ? pct(c.metrics.openRate) : '—'
            : c.metrics.replyRate ? pct(c.metrics.replyRate) : '—'}
        </span>
      ),
    },
    {
      ...
      header: 'Click rate',
      render: (c) => (
        <span className="text-ink-muted">{c.metrics.clickRate ? pct(c.metrics.clickRate) : '—'}</span>
      ),
    },
    {
      key: 'sentAt',
      header: 'Sent',
      render: (c) => (
        <span className="text-ink-subtle">{c.sentAt ? dateLabel(c.sentAt) : '—'}</span>
      ),
    },
  ];
  ...
  return (
    <div data-tour="marketing.page">
      <PageHeader
        ...
        actions={
          <Button data-tour="marketing.newCampaign" onClick={() => setWizardOpen(true)}>
      ...
      <div className="space-y-4 px-5 pb-8 pt-4">
        ...
        <div data-tour="marketing.summary" className="grid grid-cols-2 gap-3 lg:grid-cols-5">
          ...
          <MiniStat
            label={type === 'email' ? 'Avg open rate' : 'Avg reply rate'}
            value={pct(avgOpen)}
          />
        ...
        <div data-tour="marketing.tabs">
          <Tabs
            tabs={CAMPAIGN_TABS.map((t) => ({
              ...t,
              count:
                t.id === 'email' ? emailCampaigns.length
                : t.id === 'sms' ? smsCampaigns.length
                : undefined,
            }))}
            active={activeTab}
        ...
        {activeTab === 'templates' ? (
          ...
          <>
                ...
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${
                    statusFilter === f.id
                      ...
                      : 'border border-line bg-surface-sunken text-ink-muted hover:text-ink'
                  }`}
                >
                  {f.label}
                </button>
            ...
            {/* Campaign list */}
            <div
              data-tour="marketing.campaignList"
            ...
            >
              <SimpleTable
                ...
                empty={`No ${type} campaigns match the current filter.`}
              />
            </div>
          </>
        )}
