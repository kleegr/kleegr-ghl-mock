/**
 * Sites — funnels, websites, forms, surveys, and submission tracking.
 * Demo Mode: local static data only (no store entities for sites/funnels).
 * Submissions derived from store contacts.
 *
 * data-tour targets:
...
import {
  LayoutTemplate, Globe, FileText, ClipboardList,
  Plus, MousePointerClick, Eye,
} from 'lucide-react';
...
const FORMS = [
  { id: 'frm1', name: 'Contact Us',             type: 'Form',   submissions: 142, convRate: '18%', lastSubmission: '2026-05-28', status: 'Active' },
  { id: 'frm2', name: 'Booking Intake',          type: 'Form',   submissions:  89, convRate: '24%', lastSubmission: '2026-05-27', status: 'Active' },
  { id: 'frm3', name: 'New Client Onboarding',   type: 'Form',   submissions:  34, convRate: '41%', lastSubmission: '2026-05-25', status: 'Active' },
  { id: 'frm4', name: 'Referral Form',           type: 'Form',   submissions:  22, convRate: '12%', lastSubmission: '2026-05-20', status: 'Draft'  },
  { id: 'srv1', name: 'NPS Survey',              type: 'Survey', submissions:  68, convRate: '32%', lastSubmission: '2026-05-26', status: 'Active' },
  { id: 'srv2', name: 'Post-Appointment Feedback', type: 'Survey', submissions: 51, convRate: '27%', lastSubmission: '2026-05-24', status: 'Active' },
  { id: 'srv3', name: 'Service Interest Poll',   type: 'Survey', submissions:  29, convRate: '19%', lastSubmission: '2026-05-15', status: 'Draft'  },
];
      ...
      <div data-tour="sites.builderPreview" className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-xl bg-surface-sunken p-3 text-center">
            <p className="text-xl font-extrabold text-ink">{item.visits.toLocaleString()}</p>
            <p className="text-xs text-ink-muted">Visits</p>
          </div>
          <div className="rounded-xl bg-surface-sunken p-3 text-center">
            <p className="text-xl font-extrabold text-ink">{item.conversions.toLocaleString()}</p>
            <p className="text-xs text-ink-muted">Conversions</p>
          </div>
          <div className="rounded-xl bg-surface-sunken p-3 text-center">
            <p className="text-xl font-extrabold text-good">{convRate}</p>
            <p className="text-xs text-ink-muted">Conv. rate</p>
          </div>
        </div>
              ...
              {steps.map((s, i) => (
                <div key={i} className="flex items-center gap-3 rounded-lg border border-line bg-surface-sunken p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-ink-muted">{s.visits.toLocaleString()} visits</p>
                  </div>
        ...
        >
          <div className="flex items-start justify-between">
            <span className="grid h-9 w-9 place-items-center rounded-lg bg-brand-soft text-brand">
              {f.type === 'Form' ? <FileText size={16} /> : <ClipboardList size={16} />}
            </span>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 border-t border-line pt-3 text-center text-xs">
            ...
            <div><p className="font-bold text-ink">{f.lastSubmission.slice(5)}</p><p className="text-ink-subtle">Last sub.</p></div>
          </div>
        </Card>
      ))}
    </div>
      ...
      <table className="w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-line text-left">
            {['Submitter', 'Form / Survey', 'Date', 'Source', 'Status'].map((h) => (
              <th
                key={h}
        ...
        <tbody>
          {rows.map((s) => (
            <tr
              key={s.id}
              className="cursor-pointer border-b border-line/70 hover:bg-surface-sunken"
            >
              ...
              <td className="px-4 py-3">
                <Badge tone={s.status === 'Processed' ? 'good' : 'warn'}>{s.status}</Badge>
              </td>
            </tr>
          ))}
        </tbody>
...
export function Sites() {
  ...
  const renderCard = (item: SiteItem) => {
    ...
    const subText  = isFunnel
      ? `${(item as typeof FUNNELS[0]).steps} steps · Funnel`
      : `${(item as typeof WEBSITES[0]).pages} pages · Website`;
      ...
      >
        <div className="flex items-start justify-between">
          <span className="grid h-10 w-10 place-items-center rounded-lg bg-brand-soft text-brand">
            {isFunnel ? <LayoutTemplate size={18} /> : <Globe size={18} />}
          </span>
          <Badge tone={item.status === 'Published' ? 'good' : 'neutral'}>{item.status}</Badge>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between border-t border-line pt-3 text-xs">
          <span className="flex items-center gap-1 text-ink-muted">
            <MousePointerClick size={12} /> {item.visits.toLocaleString()} visits
          </span>
  ...
  return (
    <div data-tour="sites.page">
      <PageHeader
        ...
        actions={
          <Button
            data-tour="sites.newSite"
            onClick={() =>
              pushToast({
                title: 'Demo: builder opening',
                description: 'The funnel/site builder would launch here (demo only).',
                variant: 'info',
              })
            }
          >
      ...
      <div className="space-y-4 px-5 pb-8 pt-4">
        ...
        {/* Tabs */}
        <div data-tour="sites.tabs">
          <Tabs tabs={SITES_TABS} active={activeTab} onChange={setActiveTab} />
        </div>

        {/* Funnels grid */}
        {activeTab === 'funnels' && (
          <div data-tour="sites.grid" className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
