/**
 * Reporting — performance across attribution, appointments, calls, campaigns, revenue, agents.
 * Demo Mode: KPIs derived from store; charts use store data + local static trend arrays.
 *
 * data-tour targets:
...
import { Download, Sparkles, DollarSign, Users, Calendar, Phone, Star, Mail } from 'lucide-react';
...
// ─── Static trend datasets (demo only, internally consistent with seed) ───────
...
export function Reporting() {
  ...
  const avgOpen     = sentCamps.length
    ? sentCamps.reduce((s, c) => s + (c.metrics.openRate ?? 0), 0) / sentCamps.length
    : 0;
    ...
    <div data-tour="reporting.page">
        ...
        actions={
          <div className="flex items-center gap-2">
              ...
              {DATE_RANGES.map((d) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1 text-xs font-semibold transition-colors ${
                    dateRange === d.id ? 'bg-brand text-brand-fg' : 'text-ink-muted hover:text-ink'
                  }`}
                >
                  {d.label}
                </button>
            ...
            <Button
              ...
              onClick={() =>
                pushToast({ title: 'Demo: export', description: 'CSV would download here (demo only).', variant: 'info' })
              }
            >
      ...
      <div className="space-y-4 px-5 pb-8 pt-4">
        ...
        <div data-tour="reporting.kpis" className="grid grid-cols-2 gap-3 lg:grid-cols-4 xl:grid-cols-7">
          {kpis.map((k) => (
            <MiniStat key={k.label} label={k.label} value={k.value} sub={k.sub} />
          ))}
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-ink-muted">
                Your pipeline is trending positively — won revenue is up{' '}
                <span className="font-semibold text-good">19%</span> versus the prior period.
                Appointment show rates are solid at{' '}
                <span className="font-semibold text-ink">{showRate}%</span>. Email open rates of{' '}
                <span className="font-semibold text-ink">{pct(avgOpen)}</span> exceed the industry
                average of ~35%. Consider following up on the{' '}
                <span className="font-semibold text-ink">{opps.filter((o) => o.status === 'open').length}</span>{' '}
        ...
        <div data-tour="reporting.charts">
                  ...
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr className="border-b border-line text-left">
                        {['Source', 'Leads', 'Share', 'Opportunities', 'Conv. rate'].map((h) => (
                          <th key={h} className="whitespace-nowrap bg-surface px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink-subtle">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {leadSources.map((ls) => {
                        const totalLeads = leadSources.reduce((s, l) => s + l.value, 0);
                        const oppsFromSource = opps.filter(
                          (o) => contacts.find((c) => c.id === o.contactId)?.source === ls.source,
                        );
                        return (
                          <tr key={ls.source} className="border-b border-line/70 hover:bg-surface-sunken">
                            ...
                            <td className="px-4 py-3 text-ink-muted">{totalLeads ? pct(ls.value / totalLeads) : '—'}</td>
                            <td className="px-4 py-3 text-ink-muted">{oppsFromSource.length}</td>
                            <td className="px-4 py-3 text-ink-muted">
                              {ls.value ? pct(oppsFromSource.length / ls.value) : '—'}
                            </td>
                          </tr>
                        );
                      })}
          ...
          {activeTab === 'calls' && (
            <div className="space-y-4">
              ...
              <Card>
                <CardHeader title="Direction breakdown" />
                <div data-tour="reporting.table" className="p-4">
                  <div className="grid grid-cols-3 gap-4">
                    {[
                      { label: 'Inbound',  count: calls.filter((c) => c.direction === 'inbound').length,  color: 'text-brand' },
                      { label: 'Outbound', count: calls.filter((c) => c.direction === 'outbound').length, color: 'text-good'  },
                      { label: 'Missed',   count: missedCalls, color: 'text-bad'   },
                    ].map((d) => (
          ...
          {activeTab === 'campaigns' && (
            <div className="space-y-4">
              <Card>
                ...
                <div className="h-64 p-4">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={CAMPAIGN_TREND} margin={{ left: -10, right: 6 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#eef0f3" vertical={false} />
                      <XAxis dataKey="m" tick={TICK} axisLine={false} tickLine={false} />
              ...
              <Card>
                ...
                <div data-tour="reporting.table" className="overflow-x-auto">
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr className="border-b border-line text-left">
                        {['Name', 'Type', 'Audience', 'Delivered', 'Open / Reply', 'Clicks', 'Sent'].map((h) => (
                          <th key={h} className="whitespace-nowrap bg-surface px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink-subtle">{h}</th>
                        ))}
                      </tr>
                    </thead>
          ...
          {activeTab === 'revenue' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <MiniStat label="Collected (invoices)" value={money(paidRevenue)} />
              </div>
            </div>
          )}
            ...
            <Card>
              ...
              <div data-tour="reporting.table" className="overflow-x-auto">
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-line text-left">
                      {['Agent', 'Leads owned', 'Deals won', 'Win rate', 'Revenue'].map((h) => (
                        <th key={h} className="whitespace-nowrap bg-surface px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-ink-subtle">{h}</th>
                      ))}
                    </tr>
                  </thead>
